...
def SVC_sensitivity(df, scores):
    ...
    vectorizer = [
        #Baseline
        StemmedTfidfVectorizer(norm='l2', encoding='utf-8', 
                                        stop_words='english', ngram_range = (1,2),
                                        max_df = .8, min_df = 3, max_features=10000,
                                        strip_accents = 'ascii'),
        
        StemmedCountVectorizer(encoding='utf-8', 
                                        stop_words='english', ngram_range = (1,2),
                                        max_df = .8, min_df = 3, max_features=10000,
                                        strip_accents = 'ascii'),
        
        #Keep stop words
        StemmedTfidfVectorizer(norm='l2', encoding='utf-8', ngram_range = (1,2),
                                        max_df = .8, min_df = 3, max_features=10000,
                                        strip_accents = 'ascii'),
        
        StemmedCountVectorizer(encoding='utf-8', ngram_range = (1,2),
                                        max_df = .8, min_df = 3, max_features=10000,
                                        strip_accents = 'ascii'),
        
        #Unigrams Only
        StemmedTfidfVectorizer(norm='l2', encoding='utf-8', ngram_range = (1,1), 
                                        stop_words = 'english',
                                        max_df = .8, min_df = 3, max_features=10000,
                                        strip_accents = 'ascii'),
        
        StemmedCountVectorizer(encoding='utf-8', ngram_range = (1,1), 
                                        stop_words = 'english',
                                        max_df = .8, min_df = 3, max_features=10000,
                                        strip_accents = 'ascii'),
        #No Stemming
        TfidfVectorizer(norm='l2', encoding='utf-8', ngram_range = (1,1), stop_words = 'english',
                                        max_df = .8, min_df = 3, max_features=10000,
                                        strip_accents = 'ascii'),
        
        CountVectorizer(encoding='utf-8', ngram_range = (1,1), stop_words = 'english',
                                        max_df = .8, min_df = 3, max_features=10000,
                                        strip_accents = 'ascii')
        ]
    
    vec_name = ['TF-IDF Baseline', 'BoW Baseline', 
    ...
    for score in scores:
        for vec in vectorizer:
            ...
            X_train, X_test, y_train, y_test = train_test_split(features, labels, random_state = 1234,test_size=0.3)
    
            SVC_BestParams = SVC_gridsearch_sens(score, X_train, y_train)
            
          
            clf = SVC(**SVC_BestParams).fit(X_train, y_train)
            cv_scores = cross_val_score(clf, X_train, y_train, cv = 5, scoring = score)
            cv_mean.append(cv_scores.mean())
            cv_std.append(cv_scores.std())
            score_name.append(score)
            pred1 = clf.predict(X_test)
            if score == 'precision':
                Out_score.append(precision_score(y_test, pred1, average="macro"))
...
def pre_process_plots(preprocessing_scores, scores):
    ...
    for score in scores: 
        ...
        plt.savefig('Plots/' + score +'_sensitivity.png', bbox_inches = "tight")
        plt.show()
